...
{
	echo "Error: PHP 'Tokenizer' extension is missing or not enabled.";
	exit(1);
}
if (!extension_loaded("mbstring"))
{
	echo "Error: PHP 'mbstring' extension is missing or not enabled.";
	exit(1);
}

// real path to subl packages dir
$subl_plugin_path = dirname(realpath(__FILE__));
...
foreach ($files as $file)
{
	...
	if ($encoding and !mb_check_encoding($source_orig, $encoding))
	{
		...
		if (!file_put_contents(TEMPFILE, $source_converted))
		{
			echo "Error: The temporary file '".TEMPFILE."' could not be saved.";
			exit(1);
		}
...
function phptidy($source)
{
	...
	// Simple formatting
	if ($GLOBALS['fix_token_case'])
	{
		fix_token_case($tokens);
	}
	if ($GLOBALS['fix_builtin_functions_case'])
	{
		fix_builtin_functions_case($tokens);
	}
	if ($GLOBALS['replace_inline_tabs'])
	{
		replace_inline_tabs($tokens);
	}
	if ($GLOBALS['replace_phptags'])
	{
		replace_phptags($tokens);
	}
	if ($GLOBALS['replace_shell_comments'])
	{
		replace_shell_comments($tokens);
	}
	if ($GLOBALS['fix_statement_brackets'])
	{
		fix_statement_brackets($tokens);
	}
	if ($GLOBALS['fix_separation_whitespace'])
	{
		fix_separation_whitespace($tokens);
	}
	if ($GLOBALS['fix_comma_space'])
	{
		fix_comma_space($tokens);
	}
	if ($GLOBALS['fix_round_bracket_space'])
	{
		fix_round_bracket_space($tokens);
	}
	...
	if ($GLOBALS['add_file_docblock'])
	{
		add_file_docblock($tokens);
	}
	if ($GLOBALS['add_function_docblocks'])
	{
	...
	if ($GLOBALS['fix_docblock_format'])
	{
		fix_docblock_format($tokens);
	}
	if ($GLOBALS['fix_docblock_space'])
	{
...
function print_tokens($tokens)
{
	foreach ($tokens as $token)
	{
		...
		{
			list($id, $text) = $token;
			echo token_name($id)." ".addcslashes($text, "\0..\40!@\@\177..\377").PHP_EOL;
		}
	}
}
...
function possible_syntax_error($tokens, $key, $message = "")
{
	...
	echo ":".PHP_EOL;
	echo combine_tokens(array_slice($tokens, max(0, $key-5), 10)).PHP_EOL;
}
...
function replace_phptags(&$tokens)
{
	foreach ($tokens as $key => &$token)
	{
		...
		case T_OPEN_TAG:
			...
			if (rtrim($token[1]) == $GLOBALS['open_tag'])
			{
				continue;
			}
...
function fix_statement_brackets(&$tokens)
{
	...
	foreach ($tokens as $key => &$token)
	{
		if (is_string($token) or !in_array($token[0], $statement_tokens))
		{
			continue;
		}
		...
		if (!count($tokens_arg) or $tokens_arg[0] !== "(")
		{
			continue;
		}
	...
	foreach ($tokens as $key => &$token)
	{
		...
		else
		{
			...
			case T_ELSEIF:
				...
				{
					// Set the existing whitespace before the bracket to exactly one space or a newline
					$tokens[$key+1][1] = separation_whitespace(T_ELSEIF);
				}
...
function fix_docblock_format(&$tokens)
{
	foreach ($tokens as $key => &$token)
	{
		if (is_string($token) or $token[0] !== T_DOC_COMMENT)
		{
			continue;
		}
		...
		foreach ($lines_orig as $line)
		{
			...
			if (!$comments_started)
			{
				if ($line == "*" and count($lines_orig)>1)
				{
					continue;
				}
...
function add_blank_lines(&$tokens)
{
	...
	foreach ($tokens as $key => &$token)
	{
		...
		if ($token === "}")
		{
			...
			if (in_array($curly_brace_opener[$curly_braces_count], array(T_IF, T_ELSEIF, T_WHILE, T_FOR, T_FOREACH, T_SWITCH, T_FUNCTION, T_CLASS)))
			{
				if (isset($tokens[$key+1][0]) and isset($tokens[$key+1][1]))
				{
					// At least "1" blank lines after a function or class
...
function add_file_docblock(&$tokens)
{
	...
	elseif ($tokens[0][0] == T_OPEN_TAG)
	{
		...
		if ($GLOBALS['open_tag'] == "<?")
		{
			if ($tokens[1][0] === T_WHITESPACE and $tokens[2][0] === T_DOC_COMMENT)
			{
				return;
			}
	...
	elseif ($tokens[0][0] == T_INLINE_HTML)
	{
		if (preg_match("/^#!\//", $tokens[0][1]))
		{
			...
			if ($GLOBALS['open_tag'] == "<?")
			{
				if ($tokens[2][0] === T_WHITESPACE and $tokens[3][0] === T_DOC_COMMENT)
				{
					return;
				}
				// Insert new file docblock after open tag
...
function add_function_docblocks(&$tokens)
{
	foreach ($tokens as $key => &$token)
	{
		if (is_string($token) or $token[0] !== T_FUNCTION)
		{
			continue;
		}

		// No DocBlock for anonymous functions
		if (isset($tokens[$key+1]) and $tokens[$key+1] === "(")
		{
			continue;
		}
...
function add_doctags(&$tokens, $usetags, $paramtags, $returntags, $seetags)
{
	...
	foreach ($tokens as $key => &$token)
	{
		...
		$k = $key + 1;
		while (isset($tokens[$k][0]) and in_array($tokens[$k][0], array(T_WHITESPACE, T_STATIC, T_PUBLIC, T_PROTECTED, T_PRIVATE)))
		{
			++$k;
		}
		if (
